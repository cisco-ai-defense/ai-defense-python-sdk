...
class HttpInspectionClient(InspectionClient):
    ...
    def inspect_http_api(self,
                        ...
                        request_id: Optional[str] = None) -> InspectResponse:
        """
        ...
        """
        self.config.logger.debug(f"inspect_http_api called | http_req: {http_req}, http_res: {http_res}, http_meta: {http_meta}, metadata: {metadata}, config: {config}, request_id: {request_id}")
        ...
        def ensure_base64_body(d):
            ...
            return self._inspect(req, res, meta, metadata, config, request_id=request_id)

    def inspect_request_from_http_library(self,
                                         http_request: Any,
                                         metadata: Optional[Metadata] = None,
                                         config: Optional[InspectionConfig] = None,
                                         request_id: Optional[str] = None) -> InspectResponse:
        ...
        """
        self.config.logger.debug(f"inspect_request_from_http_library called | http_request: {http_request}, metadata: {metadata}, config: {config}, request_id: {request_id}")
        method = None
        headers = {}
        body = b""
        url = None
        ...
        if not body:
            body = getattr(http_request, 'body', b"") or getattr(http_request, 'content', b"")
            if isinstance(body, str):
                body = body.encode()
        if not url:
            url = getattr(http_request, 'url', None)
        ...
        if not isinstance(body, (bytes, type(None))):
            raise ValueError("HTTP request body must be bytes or None for base64 encoding.")
        from .utils import to_base64_bytes
        http_req = HttpReqObject(method=method, headers=HttpHdrObject(hdrKvs=hdr_kvs), body=to_base64_bytes(body or b"", logger=self.config.logger))
        http_meta = HttpMetaObject(url=url or "")
        return self._inspect(http_req, None, http_meta, metadata, config, request_id=request_id)

    def inspect_response_from_http_library(self,
                                          http_response: Any,
                                          metadata: Optional[Metadata] = None,
                                          config: Optional[InspectionConfig] = None,
                                          request_id: Optional[str] = None) -> InspectResponse:
        ...
        Returns:
            InspectResponse: Inspection result.
        """
        self.config.logger.debug(f"inspect_response_from_http_library called | http_response: {http_response}, metadata: {metadata}, config: {config}, request_id: {request_id}")
        status_code = None
        headers = {}
        body = b""
        url = None
        ...
        http_res = HttpResObject(statusCode=status_code, headers=HttpHdrObject(hdrKvs=hdr_kvs), body=body_b64)
        # Build http_req from associated request if possible
        http_req = None
        if http_request is not None:
            method = getattr(http_request, 'method', None)
            req_headers = dict(getattr(http_request, 'headers', {}))
            req_body = getattr(http_request, 'body', b"") or getattr(http_request, 'content', b"")
            ...
            http_req = HttpReqObject(method=method, headers=HttpHdrObject(hdrKvs=req_hdr_kvs), body=req_body_b64)
        # If http_req could not be built, raise a clear error
        if http_req is None:
            raise ValueError("Could not extract HTTP request context from response object. 'http_req' is required for inspection.")
    ...
    def inspect_request(self,
        ...
        hdr_kvs = [self._header_to_kv(k, v) for k, v in (headers or {}).items()]
        http_req = HttpReqObject(method=method, headers=HttpHdrObject(hdrKvs=hdr_kvs), body=body_b64)
        http_meta = HttpMetaObject(url=url)
    ...
    def inspect_response(
        ...
        """
        self.config.logger.debug(f"inspect_response called | status_code: {status_code}, url: {url}, headers: {headers}, body: {body}, request_method: {request_method}, request_headers: {request_headers}, request_body: {request_body}, request_metadata: {request_metadata}, metadata: {metadata}, config: {config}, request_id: {request_id}")
        ...
        if request_method or request_headers or request_body or request_metadata:
            req_hdr_kvs = [self._header_to_kv(k, v) for k, v in (request_headers or {}).items()]
            if request_body is None:
                req_body_b64 = ""
            elif isinstance(request_body, str):
                req_body_b64 = to_base64_bytes(request_body.encode(), logger=self.config.logger)
            else:
                req_body_b64 = to_base64_bytes(request_body, logger=self.config.logger)
            http_req = HttpReqObject(method=request_method, headers=HttpHdrObject(hdrKvs=req_hdr_kvs), body=req_body_b64)
    ...
    def _inspect(self, http_req, http_res, http_meta, metadata, config, entities_map=None, request_id: Optional[str] = None) -> InspectResponse:
        """
        ...
        """
        self.config.logger.debug(f"_inspect called | http_req: {http_req}, http_res: {http_res}, http_meta: {http_meta}, metadata: {metadata}, config: {config}, entities_map: {entities_map}, request_id: {request_id}")
        ...
        if not config.enabled_rules:
            if entities_map:
                ...
                config.enabled_rules = [
                    Rule(rule_name=rn, entity_types=entities_map.get(rn)) for rn in RuleName
                ]
            else:
    ...
    def _prepare_request_data(self, request: HttpInspectRequest) -> Dict[str, Any]:
        ...
        request_dict = {}
        if request.http_req:
            request_dict[HTTP_REQ] = convert(request.http_req, logger=self.config.logger)
        if request.http_res:
            request_dict[HTTP_RES] = convert(request.http_res, logger=self.config.logger)
        if request.http_meta:
            request_dict[HTTP_META] = convert(request.http_meta, logger=self.config.logger)
        if request.metadata:
            request_dict["metadata"] = convert(request.metadata, logger=self.config.logger)
        if request.config:
    ...
    def validate_inspection_request(self, request_dict: Dict[str, Any]):
        ...
        if config is not None:
            if not config.get('enabled_rules') or not isinstance(config['enabled_rules'], list):
                raise ValidationError("config.enabled_rules must be a non-empty list of Rule objects.")
