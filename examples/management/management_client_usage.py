...
from aidefense.management.models.connection import (
    ConnectionType, EditConnectionOperationType, CreateConnectionRequest,
...
from aidefense.management.models.policy import (
    ListPoliciesRequest, AddOrUpdatePolicyConnectionsRequest
)
...
def main():
    ...
    if not management_api_key:
        ...
        print("Please set the AIDEFENSE_MANAGEMENT_API_KEY environment variable to your AI Defense Management API key.")
        return
    ...
    try:
        ...
        try:
            if not created_app_id:
                ...
                connection_name = f"Test Connection {datetime.now().strftime('%Y%m%d%H%M%S')}"
                ...
                create_conn_request = CreateConnectionRequest(
                    ...
                    key={
                        "name": key_name,
                        "expiry": (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%dT%H:%M:%SZ")
                    }
                )
                ...
                if result.key:
                    ...
                    print(f"API Key: {generated_api_key[:5]}...{generated_api_key[-5:]} (masked for security)")
        ...
        try:
            if not created_connection_id:
                ...
                if generated_api_key:
                    print(f"Using previously generated API Key: {generated_api_key[:5]}...{generated_api_key[-5:]} (masked for security)")
        except ValidationError as e:
            print(f"Validation error: {e}")
        except ApiError as e:
            print(f"API error: {e}")
        ...
        try:
            if not generated_api_key:
                ...
                chat_inspection_client = ChatInspectionClient(api_key=generated_api_key, config=config)
                ...
                inspection_result = chat_inspection_client.inspect_conversation(conversation)
            ...
            for policy in policies.items:
                ...
                if policy.guardrails and policy.guardrails.items:
                    ...
                    for guardrail in policy.guardrails.items:
                        print(f"      - {guardrail.name}: {guardrail.status} (Action: {guardrail.action})")
                ...
                if policy.connections and policy.connections.items:
                    ...
                    for connection in policy.connections.items:
                        print(f"      - {connection.connection_id}: {connection.connection_name}")
        ...
        try:
            ...
            print(f"Listing events from {start_time.strftime('%Y-%m-%dT%H:%M:%SZ')} to {end_time.strftime('%Y-%m-%dT%H:%M:%SZ')}...")
            ...
            if events.items:
                ...
                for event in events.items:
                    print(f"  - {event.event_id}: {event.event_date} - {event.event_action}")
                ...
                if events.items:
                    ...
                    event_detail = client.events.get_event(first_event.event_id, expanded=True)
                    ...
                    conversation = client.events.get_event_conversation(first_event.event_id, expanded=True)
                    ...
                    if "messages" in conversation and conversation["messages"].items:
                        ...
                        for msg in conversation["messages"].items:
                            ...
                            print(f"  - {msg.direction} ({msg.role}): {content_preview}")
            else:
