...
# Provider counts per framework
declare -A PROVIDER_COUNTS
PROVIDER_COUNTS["langgraph-agent"]=4
PROVIDER_COUNTS["langchain-agent"]=4
PROVIDER_COUNTS["openai-agent"]=2
PROVIDER_COUNTS["autogen-agent"]=4
PROVIDER_COUNTS["crewai-agent"]=4
PROVIDER_COUNTS["strands-agent"]=4
...
for framework in "${FRAMEWORKS_TO_RUN[@]}"; do
    if [ -n "$QUICK" ]; then
        ...
        TOTAL_TESTS=$((TOTAL_TESTS + PROVIDER_COUNTS[$framework] * MODE_MULTIPLIER))
    fi
done
